...
public class MyCollectFragment extends Fragment {
    ...
    void initData() {
        // 列表
        VideosApi.getInstance().userCollectVideos(App.myUserId, page).enqueue(new Callback<VideoApiDto.UserVideosResponse>() {
            @Override
            public void onResponse(Call<VideoApiDto.UserVideosResponse> call, Response<VideoApiDto.UserVideosResponse> response) {
                ...
                if (res.List.size() == 0) {
                    ...
                    binding.noData.text.setText(getString(R.string.noCollectVideo));
                    return;
                }
        ...
        // 数量
        String count = getString(R.string.video_count).toString().replace("?", String.valueOf(App.myCollectVideoNum));
