...
class MySupportListFragment(private val activity: Activity) : Fragment() {
    ...
    fun initData() {
        // 列表
        UsersApi.getInstance().mySupportList(page) { res: MyUserListRes ->
            userList = res.List
            initView()
        }
        // 数量
        ...
        binding.titleCount.text = activity.getString(R.string.totalFansNum).replace("?", App.followNum.toString())
    }
