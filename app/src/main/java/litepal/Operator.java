...
import litepal.FluentQuery;
import litepal.LitePalApplication;
import litepal.LitePalDB;
import litepal.crud.DeleteHandler;
import litepal.crud.LitePalSupport;
import litepal.crud.QueryHandler;
import litepal.crud.SaveHandler;
import litepal.crud.UpdateHandler;
import litepal.crud.async.AverageExecutor;
import litepal.crud.async.CountExecutor;
import litepal.crud.async.FindExecutor;
import litepal.crud.async.FindMultiExecutor;
import litepal.crud.async.SaveExecutor;
import litepal.crud.async.UpdateOrDeleteExecutor;
...
import litepal.tablemanager.callback.DatabaseListener;
import litepal.util.BaseUtility;
import litepal.util.Const;
import litepal.util.DBUtility;
import litepal.util.SharedUtil;
import litepal.util.cipher.CipherUtil;
 ...
 * work. Also you can switch the using database by calling {@link #use(litepal.LitePalDB)} and {@link #useDefault()}
 * methods.
 *
 * @author Tony Green
 * @since 2.1
...
public class Operator {
    ...
    public static void initialize(Context context) {
        litepal.LitePalApplication.sContext = context;
    }
    ...
    public static void use(LitePalDB litePalDB) {
        synchronized (LitePalSupport.class) {
            ...
            // set the extra key name only when use database other than default or dbconfig.xml not exists
    ...
    /**
     * Switch the using database to default with configuration by dbconfig.xml.
     */
    ...
    public static boolean deleteDatabase(String dbName) {
        synchronized (LitePalSupport.class) {
            if (!TextUtils.isEmpty(dbName)) {
                ...
                File dbFile = litepal.LitePalApplication.getContext().getDatabasePath(dbName);
                if (dbFile.exists()) {
    ...
    /**
     * Check the dbName is default database or not. If it's same as dbName in dbconfig.xml, then it is
     * default database.
     *
     ...
     */
    public static litepal.FluentQuery select(String... columns) {
        litepal.FluentQuery cQuery = new litepal.FluentQuery();
     ...
     * @return A FluentQuery instance.
     */
    public static litepal.FluentQuery where(String... conditions) {
        litepal.FluentQuery cQuery = new litepal.FluentQuery();
     ...
     * @return A FluentQuery instance.
     */
    public static litepal.FluentQuery order(String column) {
        litepal.FluentQuery cQuery = new litepal.FluentQuery();
     ...
     * @return A FluentQuery instance.
     */
    public static litepal.FluentQuery limit(int value) {
        litepal.FluentQuery cQuery = new litepal.FluentQuery();
     ...
     * @return A FluentQuery instance.
     */
    public static litepal.FluentQuery offset(int value) {
        litepal.FluentQuery cQuery = new litepal.FluentQuery();
    ...
    public static <T> T sum(String tableName, String columnName, Class<T> columnType) {
        synchronized (LitePalSupport.class) {
            litepal.FluentQuery cQuery = new FluentQuery();
