...
/**
 * This is the content handler for analysis the dbconfig.xml file by SAXParser,
...
public class LitePalContentHandler extends DefaultHandler {
     ...
     */
    private litepal.parser.LitePalAttr litePalAttr;
    ...
    /**
     * Start analysis the dbconfig.xml file. Set all the parsed value into the
     * LitePalAttr model.
     */
    @Override
    public void startElement(String uri, String localName, String qName, Attributes attributes)
            throws SAXException {
        if (litepal.parser.LitePalParser.NODE_DB_NAME.equalsIgnoreCase(localName)) {
            for (int i = 0; i < attributes.getLength(); i++) {
                if (litepal.parser.LitePalParser.ATTR_VALUE.equalsIgnoreCase(attributes.getLocalName(i))) {
                    litePalAttr.setDbName(attributes.getValue(i).trim());
                }
            }
        } else if (litepal.parser.LitePalParser.NODE_VERSION.equalsIgnoreCase(localName)) {
            for (int i = 0; i < attributes.getLength(); i++) {
                if (litepal.parser.LitePalParser.ATTR_VALUE.equalsIgnoreCase(attributes.getLocalName(i))) {
                    litePalAttr.setVersion(Integer.parseInt(attributes.getValue(i).trim()));
                }
            }
        } else if (litepal.parser.LitePalParser.NODE_MAPPING.equalsIgnoreCase(localName)) {
            for (int i = 0; i < attributes.getLength(); i++) {
                if (litepal.parser.LitePalParser.ATTR_CLASS.equalsIgnoreCase(attributes.getLocalName(i))) {
                    litePalAttr.addClassName(attributes.getValue(i).trim());
                }
            }
        } else if (litepal.parser.LitePalParser.NODE_CASES.equalsIgnoreCase(localName)) {
            for (int i = 0; i < attributes.getLength(); i++) {
                if (litepal.parser.LitePalParser.ATTR_VALUE.equalsIgnoreCase(attributes.getLocalName(i))) {
                    litePalAttr.setCases(attributes.getValue(i).trim());
                }
            }
        } else if (litepal.parser.LitePalParser.NODE_STORAGE.equalsIgnoreCase(localName)) {
            for (int i = 0; i < attributes.getLength(); i++) {
