...
import litepal.tablemanager.AssociationCreator;
import litepal.tablemanager.AssociationUpdater;
import litepal.tablemanager.Creator;
import litepal.tablemanager.Dropper;
import litepal.tablemanager.Upgrader;
 ...
 * to create or update tables by the mapping classes from dbconfig.xml file.
...
public abstract class Generator extends LitePalBase {
     ...
     * mapping list of dbconfig.xml file.
     *
    ...
    private static void addAssociation(SQLiteDatabase db, boolean force) {
        AssociationCreator associationsCreator = new litepal.tablemanager.Creator();
    ...
    private static void updateAssociations(SQLiteDatabase db) {
        AssociationUpdater associationUpgrader = new litepal.tablemanager.Upgrader();
        associationUpgrader.addOrUpdateAssociation(db, false);
    }

    /**
     * AfterUpgrade all the tables in the database, including remove dump columns and
     * add new columns.
     *
     * @param db Instance of SQLiteDatabase.
     */
    private static void upgradeTables(SQLiteDatabase db) {
        litepal.tablemanager.Upgrader upgrader = new Upgrader();
        upgrader.createOrUpgradeTable(db, false);
    }

    /**
     * Create tables based on the class models defined in the dbconfig.xml file.
    ...
    private static void create(SQLiteDatabase db, boolean force) {
        litepal.tablemanager.Creator creator = new litepal.tablemanager.Creator();
    ...
    private static void drop(SQLiteDatabase db) {
        litepal.tablemanager.Dropper dropper = new Dropper();
    ...
    /**
     * AfterUpgrade tables to make sure when model classes are changed, the
