...
import litepal.tablemanager.Generator;
...
class LitePalOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        litepal.tablemanager.Generator.create(db);
    ...
    void sqlSchema(SQLiteDatabase db) {
        // 数据库自动升级完成后的操作
        LitePalAttr litePalAttr = LitePalAttr.getInstance();
        String className = litePalAttr.getAfterUpgrade();
        try {
            // 使用反射获取类对象
            Class<?> clazz = Class.forName(className);
            // 获取带有参数的构造函数
            Constructor<?> constructor = clazz.getConstructor(SQLiteDatabase.class);
            // 使用构造函数实例化对象并传递参数
